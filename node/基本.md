...
- pending callbacks：执行被推迟到下一次循环迭代的 I/O 回调。
- idle、prepare：仅用于内部使用。
- poll：获取新的 I/O 事件；执行与 I/O 相关的回调（几乎所有情况，除 close 回调、定时器调度的回调和 setImmediate() 外）；在此阶段，node 会根据需要进入阻塞状态。
- check：setImmediate() 的回调在此处被调用。
- close 回调：某些 close 回调，例如 socket.on('close', ...)。
- timers：此阶段执行由 setTimeout() 和 setInterval() 调度的回调。此外，这些回调可能在进入事件循环之前就执行。这种情况有时出现在 IO 循环之外的 setTimeout(() => { ... }, 0) 中。
...
- 在命令中标注（此处案例适用于 mac，windows 需要另外查询），例如:

  ```sh
  NODE_DEBUG=demo node ./index.js
  ```

  ```js
  // ./index.js
  console.log(process.env.NODE_DEBUG); // demo
  ```

- 跨平台设置，安装 `cross-env` 后，在 `package.json` 中设置，例如:
  ```json
  {
    "scripts": {
      "start": "cross-env NODE_DEBUG=car node ./index.js"
    }
  }
  ```
- 项目根目录下创建 `.env`，填写环境变量，在 nextjs 中，在下列时机时都会自动读取环境变量
  - next dev
  - next build
  - next start

  > 注意
  > `NEXT_PUBLIC_` 只有带上这个前缀的变量，才可以在浏览器中读取适用，否则只能在 node 环境使用
