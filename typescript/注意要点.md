...
1. 基础概念
   ...
   - 泛型可以理解成一段`类型逻辑`，需要`类型参数`来表达。有了类型参数以后，可以在输入类型与输出类型之间，建立一一对应关系。
...
4. `extends`的用法
   ...
   // { [key: string]: any } 只能保证 如果有字符串属性，其值一定是 any，而不能确保一定含有 a、b
   // 因为 let a: { [key: string]: any } = {} 也是合理的
   ...
   ```ts
   // 过滤参数
   type Filter<T, U> = T extends U ? never : T; // 一个条件类型如果作用于一个联合类型，会被分配到每个成员
   type Result = Filter<string | number | boolean, number>; // Result = string | boolean
   ```

5. `infer`关键字用来定义泛型里面推断出来的类型参数，而不是外部传入的类型参数
   ...
   ```ts
   type UppercaseDemo<S extends string> = S extends `${infer Q}${infer P}`
     ? `${Uppercase<Q>}${Uppercase<P>}`
     : S;

   // 此时 S 为单个字符 a ,运行后 Q、P 会推导为 'a' 和 ''
   type UppercaseHello = UppercaseDemo<"a">;
   ```

6. `TypeScript 的 this 声明`：在 TypeScript 中，你可以显式声明 this 作为函数的第一个“形参”，但它只是类型声明，不影响实际传递的参数
...
9. `satisfies`运算符用来检测某个值是否符合指定类型。有时候，不方便将某个值指定为某种类型，但是希望这个值符合类型条件，这时候就可以用`satisfies`运算符对其进行检测。
   ...
   palette.green.substring(1, 6); // 报错，类型“string | RGB”上不存在属性“substring”。类型“RGB”上不存在属性“substring”。
   ...
   palette1.green.substring(1, 6); // 不报错
   ```
...
10. 函数参数的逆变性
    ...
    type PersonKeysIntersection = UnionToIntersection<keyof Person>; // never （"name" & "age" & "location" ts 简写为 never）
    ```

    1. 输入：`keyof Person` → `"name" | "age" | "location"`（联合类型）
    2. 第一层条件：`U extends any ? (k: U) => void : never`
       - 对联合类型的每个成员应用条件类型（分布式条件类型）
       - 结果：`(k: "name") => void | (k: "age") => void | (k: "location") => void`
       - 这是三个函数类型的联合
    3. 关键点 - 逆变性：函数参数是逆变的
       - 当把函数联合与 `(k: infer I) => void` 做比对时
       - 函数参数类型会反向：`"name" | "age" | "location"` → `"name" & "age" & "location"`
       - 推断出 I = `"name" & "age" & "location"`
    4. 返回：I 就是交叉类型
    ...
    **核心原理**
    ...
    ```
    函数参数的逆变性：
    (k: A & B) => void  可以赋给  (k: A) => void | (k: B) => void
               ↑                        ↓
            交叉类型                   联合类型
    ```
