...
7. window.ResizeObserver
   可以监听目标元素尺寸变化，用其可以在元素尺寸变化后执行回调方法
8. requestAnimationFrame
   - 是浏览器提供的一个`异步` API，用于在下一次浏览器重绘（渲染帧）前执行回调函数

   主要作用：
   - 让回调在浏览器渲染前执行，保证动画或界面更新的流畅性。
   - 比如页面滚动、元素尺寸变化、动画等场景，能避免多次无效渲染，提高性能。

   常见使用场景：
   - 实现平滑动画（如移动、渐变、缩放等）。
   - 监听和响应 DOM 元素变化（如本例中尺寸变化）。
   - 节流高频事件（如滚动、resize），只在下一帧处理一次。
   - 游戏开发中的帧同步。
   ...
   总结：  
    `requestAnimationFrame` 适合需要与浏览器渲染节奏同步的 UI 更新和动画场景。
   案例 1：canvas 动画
     ...
     <body>
       ...
       <script>
         ...
         function draw(timestamp) {
           // 清空画布
           ctx.clearRect(0, 0, canvas.width, canvas.height); // 前两参数是起点坐标，后俩参数是矩形宽高
           // 绘制移动的矩形
           ctx.fillRect(x, 50, 50, 50);
           if (x + 50 > canvas.width) {
             ctx.fillRect(0, 50, x + 50 - canvas.width, 50);
           }
...
9. Crypto
   - `Crypto` 接口提供了当前上下文中可用的基本的加密功能。它允许访问一个`密码学安全的随机数生成器`和`密码学原语`（cryptographic primitive）。
   - 实例方法
     - Crypto.randomUUID()
...
2.  `for...of`与`for...in`的区别
    - for...in
      ...
      - 包括对象自身的属性和原型链上的属性
      - 属性名是字符串或 Symbol
      - 示例：`for (let key in obj) { ... }`
    - for...of
      - 遍历**可迭代对象**的**值**（不是键）
      - 需要对象实现 Symbol.iterator 接口
      - 适用于数组、字符串、Map、Set 等
      - 示例：`for (let value of arr) { ... }`
...
8.  原型与原型链
    - 谈到继承，JavaScript 只有一种复杂数据结构：对象。
    - 每个对象内部都有一个隐藏属性 `[[Prototype]]`，指向另一个对象，这个对象称为“原型”。原型对象也可能有自己的原型，层层向上直到 `null`。`null` 没有原型，因此原型链终止于 `null`。
      这就是我们在控制台展开对象时看到的 `[[Prototype]]`。
    - `[[Prototype]]` 是一个内部属性，不能直接通过字面属性访问。常用访问方式：
      1. `obj.__proto__`：浏览器通常支持的访问器属性，历史上曾是非标准实现。现在为了兼容而被规范化，但仍建议更推荐使用下面的方法。
      2. `Object.getPrototypeOf(obj)`：更规范的方式。
    - 访问对象属性时，JavaScript 会先在自身查找，如果没有则沿原型链向上查找，直到找到属性或者到达 `null`。

    - 对于普通函数来说，它会自动获得一个 `prototype` 属性，这个属性本身是一个对象。当该函数用作构造函数并通过 `new` 创建实例时，实例的 `[[Prototype]]` 会被设置为该函数的 `prototype`。
    ...
    - `Foo.prototype` 默认有一个 `constructor` 属性，指向函数本身：
      ...
      ```
    - 函数对象本身也是对象，因此它的 `__proto__` 指向 `Function.prototype`：
      ```js
      function a() {}
...
9.  V8 引擎的垃圾回收
    ...
    3. 垃圾回收策略（如何进行）
       ...
       - 两种常见的算法策略
         1. 标记清除算法
            ...
            - 引擎在执行 GC（使用标记清除算法）时，需要从`出发点`去遍历内存中所有的对象去打标记，而这个出发点有很多，我们称之为一组`根`对象，而所谓的根对象，其实在浏览器环境中包括又不止于`全局Window对象`、`文档DOM树`、`当前调用栈变量`、`闭包中的变量`等
            - 整个标记清除算法大致过程就像下面这样
    ...
    4. V8 对 GC 的优化
       - 分代式垃圾回收
         ...
         - 新生代的对象为`存活时间较短`的对象，简单来说就是新产生的对象，通常只支持 1 ～ 8M 的容量。新生代使用 Scavenge 算法（复制算法），将内存分为两个半区（From 和 To），每次 GC 时将存活对象复制到 To 空间，清空 From 空间，然后交换角色。
         - 老生代的对象为`存活事件较长`或`常驻内存`的对象，简单来说就是经历过新生代垃圾回收后还存活下来的对象，容量通常比较大。老生代使用标记-整理算法。
         - 优点
       ...
       - 增量标记与懒性清理
         - 增量标记：将标记过程分成多个小阶段，避免长时间停顿。
         - 懒性清理：延迟清理非活动对象，优先处理高优先级任务。

       - 并发回收
         - 垃圾回收的标记阶段与应用代码并发执行，进一步减少停顿时间。

10. 闭包
    - 是由函数以及函数声明所在的词法环境组合而成的
    - 该环境包含了这个闭包创建时作用域内的任何局部变量

    - 词法环境: 当前作用域中的变量记录 + 指向外层作用域的引用
    - JS 引擎内部会为每个作用域创建一个词法环境。
    - 例如
...
13. 深拷贝
    - 概念
      - 深拷贝是指创建一个新的对象，**递归地**复制原对象中的所有属性，包括嵌套对象，新对象与原对象完全独立，修改新对象不会影响原对象
      - 与浅拷贝（只复制一层引用）相对，浅拷贝后嵌套对象仍然是同一份引用
    - 常见实现方式
      1. `JSON.parse(JSON.stringify(obj))`
         - 优点：写法简单
         - 缺点：
           - 无法处理 `undefined`、`Symbol`、`Function`（会被忽略或转为 null）
           - 无法处理 `Date`（会变成字符串）、`RegExp`（变成空对象）、`Map`、`Set` 等特殊对象
           - 无法处理循环引用（会报错）
      2. 递归深拷贝（见下方代码）
    - 递归深拷贝实现
    ...
    - 说明
      - `WeakMap` 用于缓存已拷贝的对象，遇到循环引用时直接返回缓存，避免栈溢出
      - `new origin.constructor()` 可以保留原对象的构造函数信息（如 `Array`、`Object` 等）
      - `Reflect.ownKeys()` 可同时遍历**普通属性**和 **Symbol 属性**，比 `Object.keys()` 更全面
      - 局限性：未处理 `Map`、`Set`、`WeakMap`、`WeakSet`、`ArrayBuffer` 等特殊内置对象（如需可扩展）
...
15. await
    ...
    - `await` 会阻塞`下面的`代码（注意：是下面，`await` 后面的方法还是会执行的）（即加入微任务队列），先执行 `async `外面的同步代码，同步代码执行完，再回到 `async` 函数中，再执行之前阻塞的代码
...
18. ReadableStream 解析
    浏览器 fetch API 返回的 response.body 是 ReadableStream 类型。
    - 如何解析为文本（不使用 response 下的 json()、text()、blob()方法）
    ...
    /* 
        ...
        为什么需要最后调用一次 decoder.decode()？
        在 UTF-8 编码中，一个字符可能由 1-4 个字节组成：
    
        ASCII 字符：1 字节（如 a）
        中文、表情等：2-4 字节（如 中 是 3 字节，😀 是 4 字节）
    
        如果 ReadableStream 的某一块数据刚好在多字节字符的中间截断，比如：
        块1: [0xE4, 0xB8]       // "中"字的前 2 个字节
        块2: [0xAD, 0xE5, ...]  // "中"字的最后 1 个字节 + 后续内容
    
        { stream: true } 告诉 TextDecoder：这不是最后一块数据，如果遇到不完整的字符，先缓存起来，等下一块数据来了再拼接解码
        所以块 1 处理时，[0xE4, 0xB8] 会被缓存，不会输出
        块 2 来了，[0xAD] 会和之前的缓存拼接成完整的 [0xE4, 0xB8, 0xAD]，解码为 "中"
    
        最后调用 decoder.decode() 的作用：
    
        循环结束后，如果 TextDecoder 内部还有未输出的缓存字节（比如流的最后一块数据刚好是不完整字符），调用无参数的 decoder.decode() 会：
    
        1. 刷新（flush）内部缓存
        2. 把剩余的字节按规则处理（通常会输出替换字符 � 表示损坏的数据）
    */
...
19. 手动触发事件
    ...
    ```ts
    window.dispatchEvent(new Event("resize")); // 手动触发 resize 事件
    ```

    - 示例
      - `antd@3.20.0 table` 配置固定列后，固定的那一列偶尔会和滚动部分对不齐，查阅 `rc-table` 源码后，得知其会在每次 `resize` 事件触发后重新计算，于是就想到手动出发 `resize` 事件来解决
      ```ts
      useMount(() => {
      ...
      ```
      用这个方法解决了一部分，又发现没对齐的现象，排查发现是 table 里数据的 rowKey 重复了，组件内部会计算每一列的高度用 rowKey 存储在 store 中，导致 rowKey 相同的几列高度都一致，都为其中最后一列的高度。

20. DOMContentLoaded 与 window.load
    - `DOMContentLoaded`：DOM 树构建完成后触发（页面可能还在加载图片等资源）。
    - `window.load`：页面所有资源（图片、CSS、字体、iframe 等）都加载完成后触发，而不是仅仅“页面显示后”触发。
