...
8. 原型与原型链
   ...
   - 当访问一个对象的属性时，除了会在当前对象查找，还会顺着原型链向上查找，知道找到名称符合的属性，或者查到最后的 null 也没有找到

   - 函数的 `func.prototype` 可以认作是一个空间，在使用该函数作为构造函数时，会将该属性分配给所有对象实例的 `[[prorotype]]`

   - 对象的 `__proto__`（隐式原型）等于创建它的构造函数的 prototype（显示原型）
     ```js
     function a() {}
     a.__proto__ === Function.prototype; // true
     ```
   - 函数的 prototype（显示原型） 的 constructor（构造函数） 等于函数本身
     ```js
     function a() {}
