...
- loader: 模块转换器，用于把模块原内容按照需求转换成新内容
  ...
  - 编写要点
    ...
    module.exports = function ReplaceContentLoader(source) {
      // 如果 loader 配置了 options 对象，那么this.query将指向 options
      const options = this.query;

      // 可以用作解析其他模块路径的上下文
      // 为当前处理中文件的路径，如 xxx/src/home/index.js
      console.log(this.context);

      function doSomeThing2JsString(source, key) {
        return source.replace(key, `【loader】${key}`);
      }
      const content = doSomeThing2JsString(source, this.query.key);
...
- 插件(plugins): 扩展插件，可以用来做一些 loader 无法做到的事情
  ...
  ```js
  // 生成 README.md
  module.exports = class EmitFilePlugin {
    constructor(props) {
      console.log("【EmitFilePlugin】running");
      this.options = props;
    }
    apply(compiler) {
      let _this = this;
      // 找到合适的事件钩子，实现自己的插件功能
      compiler.hooks.emit.tap("EmitFilePlugin", (compilation) => {
        // compilation: 当前打包构建流程的上下文

        compilation.assets["README.md"] = {
          source: function () {
            return _this.options.content;
          },
          size: function () {
            return _this.options.content.length;
          },
        };
      });
      // 异步
      // compiler.hooks.emit.tapAsync("EmitFilePlugin", (compilation, cb) => {
      //   compilation.assets["README.md"] = {
      //     source: function () {
      //       return _this.options.content;
      //     },
...
2. 编译构建流程：
   ...
   该对象是编译阶段的主要执行者，主要会依次进行下述流程：执行模块创建、依赖收集、分块、打包等
   ...
   ```js
    _addModuleChain(context, dependency, onModule, callback) {
        ...
        // 调用工厂函数 moduleFactory 的 create 来生成一个空的 module 对象
   ...
   - `moduleFactory.create` 方法生成一个空的 `module` 对象
