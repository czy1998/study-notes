...
git log --pretty="%C(red)%an %C(yellow)%ad %C(auto)%s" --date=format:"%Y-%m-%d %H:%M:%S" // 输出结果例如：CZY 2025-01-13 14:42:29 Merge branch
                                                                                         // 使用 %ad 后可以跟 --date=format: 来格式化
git log --grep="初始化" -i // 按照提交信息过滤，查看提交信息包含字段 "初始化" 的commit，-i 查找提交记录时忽略大小写
git log -n <number> // 展示几条日志
...
git diff // 查看工作区和最后一次提交差异
git diff --stat // 查看工作区和最后一次提交差异，并显示更改的文件数量和插入/删除的行数的统计信息
...
git remote -v // 显示所有配置的远程仓库地址。输出结果中包含了远程仓库的名称（如 origin）以及对应的 fetch 和 push URL。
git remote show origin // 显示有关指定远程仓库（通常是 origin，可以替换为实际的仓库名称）的详细信息
git remote update origin // 更新指定的远程仓库的分支列表
                         // 不传 origin 就是更新所有原创仓库的分支列表
                         // 参数
                         // -p | --prune
                         // 当远程分支被删掉后，执行此命令，会对本地仓库里该分支进行修剪，去除其远程追踪分支的信息
...
git clone <git path> // 克隆一个本地库
                     // --depth=1 只拉取仓库最新一次的 commit 记录，加快拉取速度
                     // -b dev 拉取仓库指定分支
...
git branch -u origin/test // 将当前分支与 远程 test 分支绑定，后续 pull 什么会跟随这个远程分支，可以用过 git status -bs 看到 test...origin/test 类似内容
...
git worktree add ../AA // 用源仓库在上一级新建一个名为 AA 的 worktree，
                       // 检查源仓库是否有叫 AA 的分支:
                       //   1. 没有叫 AA 的分支 → 从当前 HEAD（你在 main 上就是 main）新建 AA，再检出
                       //   2. 已经有 AA → 复用这个分支，不再新建，也不再看 main

git worktree remove ../AA // 会删除 AA 工作区，不会删除 AA 分支
git worktree list // 查看当前分支有多少工作区
