...
# 创建并运行容器
docker run -d --name screenshot_1.1.2 -p 9998:9999 --env DOMAIN=https://gjdev.workabc.com/ --env YsDomain=https://ysdev.workabc.com/ -v /Users/study/my-app-vue2:/app/vue nexus.work.com:8088/screenshot:1.1.2
# -d 后台运行容器，并返回容器 ID
# --name 指定容器名称
# -p 指定端口映射，格式为： 主机(宿主)端口:容器端口
# --env 指定环境变量，若有多个则可写多个
# -v 将本地项目的 `/Users/study/my-app-vue2` 目录（宿主机目录）挂载到容器内的 `/app/vue` 目录
#    因此，当你修改 `/Users/study/my-app-vue2` 目录下的源文件时，这些更改将直接反映在运行中的容器内。你无需重新构建容器或重启服务，查看更改只需刷新容器内运行的
#    应用程序（如在浏览器中刷新页面）。
# --platform 指定镜像的架构

docker images # 查看镜像

docker buildx build --platform linux/amd64 -t czy-demo:latest . # 构建镜像
                                                                # --platform 指定镜像的平台架构
...
docker exec -it [容器名称/容器id] sh -c 'echo $DOMAIN'
# -i 保持 STDIN 开放，即使没有附加，允许你向容器输入数据
# -t 分配一个 伪终端
# -it 以交互式终端模式运行命令，可以在容器内输入命令和接收输出，就像你直接登入了容器一样，可以进行实时交互
...
services: # 服务，可以存在多个，就像下面的 servicename、servicename2
  servicename: # 服务名字，也是内部 bridge 网络可以使用的 DNS name
    # 在集群（Swarm）模式下相当于 docker run 的时候指定的一个名称
    image: # 必选，镜像的名字
    command: # 可选，如果设置，则会覆盖默认镜像里的 CMD 命令
    entrypoint: # 可选，容器启动时执行的可执行文件或脚本；CMD 是传递给 entrypoint 的参数
    ...
    expose: # 可选，指定容器暴露的端口
    build: # 可选，Dockerfile 所在的构建目录或构建上下文路径
    depends_on: # 可选，定义服务间的启动顺序依赖关系
    env_file: # 可选，指定环境变量文件路径，从中读取环境变量
  servicename2:
    image:
    command:
    networks:
...
**使用方法**
有了 docker-compose 后，如果我们有一个服务升级了，上传了新的镜像，就可以按如下操作进行:
