...
```bash
cat > a.js # 新建文件，输入完内容，再回车，再 ctrl+d，内容会覆盖
cat >> a.js # 新建文件，输入完内容，再回车，再 ctrl+d，内容是附加
cat a.js # 查看文件
...
find # 在文件系统中按照给定的条件进行搜索
     ...
     # 果需要排除多个目录，可以在 -not \( ... \)部分、添加更多的-path条件，用,隔开即可。
...
mkdir # 创建文件夹
      # -p 递归创建目录，如果父目录不存在就一起创建；如果目录已存在也不要报错。
      # 常用案例
      # mkdir a/b 若 a 不存在，则会报错，加上 -p 后则正常运行
...
basename # 去除路径显示文件名
         ...
         # 常用案例
         # basename "git@gitee.com:xxx/medicine-up.git"
         # => hera-medicine-up
         # basename "git@gitee.com:xxx/medicine-up.git" .git
         # => hera-medicine-up
...
ps # 查看当前正在运行的进程
    ...
    # ps -p 8535 -o pid,user,lstart,cmd --width 1000 只查这个 PID，并指定输出列
    # -o pid,user,lstart,cmd：只显示 PID、启动用户、启动时间、命令
    # --width 1000：命令行尽量打全，避免被截断
...
set # 修改 shell 的运行行为
    # -e 只要有命令返回非0，脚本立即退出
    # -u 使用未定义变量时，直接报错
    # -o pipefail 管道中只要有一个命令失败，整个管道就失败
    # 常用案例
    # 常用语 shell 脚本文件开头
    # set -euo pipefail; ...(其他命令，不需要管道的话，用分号隔开)

sed # 流编辑器，利用脚本来处理文本文件
    # -n 不需要默认输出，仅显示脚本处理后的结果
    # 动作说明：
    ...
    # 常用案例
    # sed -n 's/^[[:space:]]*开发分支[：:][[:space:]]*//p'
    # 作用：替换成功后打印，匹配 开头的空格+开发分支+中英文冒号+空格，替换成空字符
    # sed 's/[[:space:]]*$//'
    # 作用：删除每行末尾空白字符，由于前面没有加 -n 命令会自动输出结果，不需要在后面再加 p 标志来输出了
    # sed '/^$/d'
    # 作用：删除空行

ss -lntp # 是看本机 正在监听的 TCP 端口，以及对应进程。
         # ss：查看 socket 连接（替代 netstat）
         # -l：只看 listening，即在等别人连进来的端口
         # -n：端口显示数字，不把 80 翻译成 http
         # -t：只看 TCP
         # -p：显示是哪个进程在听
...
uname -m # 返回当前电脑系统架构
         # mac intel => x86_64
         # mac m系列  => arm64
         # 通常可以使用 file 命令查看文件架构
         # file ./jdk/Contents/Home/bin/java => ./jdk/Contents/Home/bin/java: Mach-O 64-bit executable arm64
...
rsync # 用于远程和本地复制及同步文件和目录
      # 基础格式（归档+压缩+进度显示）
      # rsync -avzP 源路径 目标路径
      # -a 归档模式（保留所有文件属性，包括权限、时间、软链接、目录结构）
      # -v 显示详细传输过程
      # -z 传输时 gzip 压缩
      # -P 显示实时进度条
      # -e 指定远程 shell
      # 常用案例
      # rsync -avz -e "ssh -i .ssh/id_rsa" demo "root@121.121.121.121:/opt/demo"
      # -e 这里实际上是在自定义 ssh 参数
